...
available_ram equ 0x9000; 有效内存存放的位置
                        ; 存放了可用的RAM大小
pml4_entry_addr equ 0x1000
page_directory_pointer_table_addr equ 0x2000
page_directory_addr equ 0x3000
page_table_addr equ 0x4000
; hello
mov si, loading
call print
...
protect_mode:
    ...
    mov eax, 0x2000
    or eax, 0x3
    mov [pml4_entry_addr], eax

    mov eax, 0x3000
    or eax, 0x3
    mov [page_directory_pointer_table_addr], eax
...
fill_page_table:
    ...
    mov eax, 0x1000
    or eax, 0x3
    mov [page_table_addr + 8], eax
    

    mov eax, cr4
    ; enable physical-address extensions(PAE)
    or eax, 0x20
    ; A logical processer uses 4-level paging
    and eax, 0xFFFFEFFF
    mov cr4, eax

    mov eax, pml4_entry_addr
    mov cr3, eax
